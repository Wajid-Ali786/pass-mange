...
function persistData() {
  ...
  PMStorage.saveSettings({ ...PMStorage.loadSettings(), sortBy: state.sortBy, favoritesOnly: state.favoritesOnly, categoryFilter: state.categoryFilter, pageSize: state.pageSize });
  runAutoBackup();
}
...
async function saveSite() {
  ...
  if (!editingSiteId) {
    ...
    if (oldPassword !== data.password) {
      await PMHistory.addPasswordHistoryEntry(site, data.password, data.customPassDate);
      PMHistory.buildSiteHistoryEntry(site, 'password_changed', 'Password updated', data.customPassDate);
...
function toggleFavoriteSite(id) {
  ...
  setLastUsed(site);
  PMHistory.buildSiteHistoryEntry(site, site.favorite ? 'favorited' : 'unfavorited', 'Favorite toggled');
...
function togglePinSite(id) {
  ...
  PMHistory.buildSiteHistoryEntry(site, site.pinned ? 'pinned' : 'unpinned', 'Pin toggled');
  persistData();
  renderSites();
}
...
async function openPasswordHistory(id) {
  ...
  const items = await Promise.all((site.passwordHistory || []).map(async (h) => {
    const plain = await PMEncryption.decryptText(h.passwordEnc);
    const masked = plain ? `${'•'.repeat(Math.max(4, plain.length))}` : '(empty)';
    return `
  ...
  historyBody.innerHTML = items.join('') || '<p class="empty">No password history yet.</p>';
...
async function revertPassword(siteId, historyId) {
  ...
  PMHistory.buildSiteHistoryEntry(site, 'password_reverted', 'Password reverted from history');
  PMStorage.addGlobalHistory(pmGlobalHistory, site, 'password_reverted', 'Password reverted from history');
  persistData();
...
function updateCategoryUI(filteredList) {
  ...
  categoryFilterInput.innerHTML = `<option value="all">All Categories</option>${categories.map((cat) => `<option value="${PMUI.esc(cat)}">${PMUI.esc(cat)}</option>`).join('')}`;
  categoryFilterInput.value = state.categoryFilter;

  categoryListDatalist.innerHTML = categories.map((cat) => `<option value="${PMUI.esc(cat)}"></option>`).join('');

  categorySummary.innerHTML = categories.length
    ? categories.map((cat) => `<span class="category-count-pill">${PMUI.esc(cat)}: ${counts[cat]}</span>`).join('')
    ...
    card.innerHTML = `
      <h3>
        ...
        <div class="card-head-actions">
          <button onclick="togglePinSite('${site.id}')" class="ghost-btn">${site.pinned ? '<i class="fa-solid fa-thumbtack"></i>' : '<i class="fa-regular fa-thumbtack"></i>'}</button>
          ...
          <button onclick="toggleCollapseSite('${site.id}')" class="ghost-btn"><i class="fa-solid ${site.collapsed ? 'fa-angles-down' : 'fa-angles-up'}"></i></button>
        </div>
      </h3>
...
function reorderByDrag(fromId, toId) {
  const visibleManual = PMFilters.filterSites(pmSites, { ...state, favoritesOnly: false, categoryFilter: 'all', keyword: '', sortBy: 'manual' });
...
function bindEvents() {
  ...
  document.getElementById('closePasswordHistory').addEventListener('click', () => {
    historyModal.style.display = 'none';
  });
